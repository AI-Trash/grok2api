...
function downloadAccountExport(blob: Blob, name: AccountProvider | string): void {
  const url = URL.createObjectURL(blob);
  const anchor = document.createElement("a");
  anchor.href = url;
  anchor.download = name === "grok_build" || name === "grok_web" || name === "grok_console"
    ? `grok2api-${name.replaceAll("_", "-")}-accounts-${new Date().toISOString().slice(0, 10)}.json`
    : name;
  anchor.click();
