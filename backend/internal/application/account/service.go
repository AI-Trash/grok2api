...
func (s *Service) ExportProviderCredentials(ctx context.Context, providerValue accountdomain.Provider) (ExportResult, error) {
	...
	return s.exportCredentials(adapter, providerValue, values)
}
...
func (s *Service) ExportCredential(ctx context.Context, id uint64) (ExportResult, error) {
	...
	adapter, ok := s.providers.CredentialCodec(accountdomain.ProviderBuild)
	if !ok {
		return ExportResult{}, fmt.Errorf("Provider %s 不支持凭据导出", accountdomain.ProviderBuild)
	}
	return s.exportCredentials(adapter, accountdomain.ProviderBuild, []accountdomain.Credential{value})
}

func (s *Service) exportCredentials(adapter provider.CredentialCodecAdapter, providerValue accountdomain.Provider, values []accountdomain.Credential) (ExportResult, error) {
	seeds := make([]provider.CredentialSeed, 0, len(values))
	for _, value := range values {
